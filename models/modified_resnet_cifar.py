...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=10):
        ...
        self.fe_fc = modified_linear.CosineLinear(64 * block.expansion, num_classes)
    ...
    def forward(self, x, fp_mode=False):
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)

        x = self.layer1(x)
        fp1 = x.view(x.size(0), -1)
        x = self.layer2(x)
        fp2 = x.view(x.size(0), -1)
        x = self.layer3(x)
