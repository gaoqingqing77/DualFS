...
class BaseTrainer(object):
    ...
    def init_prototypes(self, dictionary_size, order, X_train_total, Y_train_total):
        ...
        alpha_dr_herding  = np.zeros((int(self.args.num_classes/self.args.nb_cl), dictionary_size, self.args.nb_cl), np.float32)#(10,500,10)/(20,500,5)/(25,500,2)
        if self.args.dataset == 'cifar100':
            # CIFAR-100, directly load the tensors for the training samples
            prototypes = np.zeros((self.args.num_classes, dictionary_size, X_train_total.shape[1], X_train_total.shape[2], X_train_total.shape[3]))#(100,500,32,32,3)
    ...
    def init_current_phase_model(self, iteration, start_iter, b1_model, b2_model, b3_model):
        ...
        Args:
          ...
          b3_model: the compression branch model from last phase
        Returns:
          b1_model: the 1st branch model from the current phase
          b2_model: the 2nd branch model from the current phase
          b3_model: the compression branch model from the current phase
          ...
          ref_b3_model: the compression branch model from last phase (frozen, not trainable)
        ...
        if iteration == start_iter:
            ...
            ref_dict = ref_model.state_dict()#state_dict将model的每一层与他对应的weights以及偏置等建立映射,构成字典对象。
            tg_dict = b1_model.state_dict()
            tg_dict.update(ref_dict)#把字典ref_dict的键/值对更新到tg_dict里。
            ...
            # Print the information about the input and output features
            print("Feature:", in_features, "Class:", out_features)#(64,50)
            new_fc = modified_linear.SplitCosineLinear(in_features, out_features, self.args.nb_cl)#(64,50,10/5/2)该函数将 第一个参数 映射到 第二个参数+第三个参数
            b1_model.fe_fc = modified_linear.SplitCosineLinear(in_features, out_features, self.args.nb_cl)
            ...
            b1_model.fc = new_fc#fc1 50 + fc2 10
            # Update the lambda parameter for the current phase
            the_lambda_mult = out_features*1.0 / self.args.nb_cl#50除以10/5/2=5/10/25
            # The 2nd branch doesn't have reference model, set it to None
            ref_b2_model = None
            # Set the compression branch for the 1st phase
            b3_model = self.network(num_classes=self.args.nb_cl_fg)
            # Load the model parameters trained last phase to the current phase model
            b3_dict = b3_model.state_dict()
            b3_dict.update(ref_dict)#导入ref_b1model的参数
            b3_model.load_state_dict(b3_dict)
            b3_model.to(self.device)
            new_b3fc = modified_linear.SplitCosineLinear(in_features, out_features, self.args.nb_cl)#int(in_features/2)
            new_b3fc.fc1.weight.data = ref_model.fc.weight.data  ###用phase0得到的fc初始化b3_model中的fc的旧类对应的权重
            new_b3fc.sigma.data = ref_model.fc.sigma.data  ###
            b3_model.fc = new_b3fc
            ref_b3_model = None
        else:
            # The i-th phase, i>=2
            # Update the index for last phase
            ...
            # Copy and freeze the compression branch model
            ref_b3_model = copy.deepcopy(b3_model)

            ref_b3_dict = ref_b3_model.state_dict()
            params_to_copy_dict = {}
            excluded_names = ['fc.sigma', 'fc.fc1.weight', 'fc.fc2.weight',
                              'fe_fc.weight', 'fe_fc.sigma']
            for name, param in ref_b3_dict.items():
                if all(name not in excluded_name for excluded_name in excluded_names):
                    params_to_copy_dict[name] = param

            b1_model.load_state_dict(params_to_copy_dict, strict=False)
            b2_model.load_state_dict(params_to_copy_dict, strict=False)
            b1_model.to(self.device)
            b2_model.to(self.device)
            ...
            b1_model.fe_fc = modified_linear.SplitCosineLinear(in_features, out_features1 + out_features2, self.args.nb_cl)
            new_b3fc = modified_linear.SplitCosineLinear(in_features, out_features1+out_features2, self.args.nb_cl)
            new_fc.fc1.weight.data[:out_features1, :] = ref_b3_model.fc.fc1.weight.data
            new_fc.fc1.weight.data[out_features1:, :] = ref_b3_model.fc.fc2.weight.data
            new_fc.sigma.data = ref_b3_model.fc.sigma.data
            b1_model.fc = new_fc
            new_b3fc.fc1.weight.data[:out_features1] = ref_b3_model.fc.fc1.weight.data
            new_b3fc.fc1.weight.data[out_features1:] = ref_b3_model.fc.fc2.weight.data
            new_b3fc.sigma.data = ref_b3_model.fc.sigma.data
            b3_model.fc = new_b3fc
            # Update the lambda parameter for the current phase
        ...
        return b1_model, b2_model, b3_model, ref_model, ref_b2_model, ref_b3_model, the_lambda_mult, cur_the_lambda, last_iter
    ...
    def init_current_phase_dataset(self, iteration, start_iter, last_iter, order, order_list, \
        ...
        indices_train_10 = np.array([i in order[range(last_iter*self.args.nb_cl,(iteration+1)*self.args.nb_cl)] for i in Y_train_total])#返回布尔值组成的数组。按照order的顺序，前50个->(50,60)类的样本为True，共500个*50类=25000个->500*10=5000个
        indices_test_10 = np.array([i in order[range(last_iter*self.args.nb_cl,(iteration+1)*self.args.nb_cl)] for i in Y_valid_total])#共100个*50类=5000个->100*10=1000
    ...
    def imprint_weights(self, b1_model, b2_model, b3_model, iteration, is_start_iteration, X_train, map_Y_train, dictionary_size):
        ...
        Returns:
          ...
          b3_model: the compression branch model from the current phase, the FC classifier is updated
        """
        if self.args.dataset == 'cifar100':
            ...
            tg_feature_model = nn.Sequential(*list(b1_model.children())[:-2])
            ...
            b3_model.to(self.device)
            #b3_model.fc.fc2.weight.data = novel_embedding.to(self.device)
        elif self.args.dataset == 'imagenet_sub' or self.args.dataset == 'imagenet':
            # Load previous FC weights, transfer them from GPU to CPU
            old_embedding_norm = b1_model.fc.fc1.weight.data.norm(dim=1, keepdim=True)
            average_old_embedding_norm = torch.mean(old_embedding_norm, dim=0).to('cpu').type(torch.DoubleTensor)
    ...
    def set_optimizer(self, iteration, start_iter, b1_model, ref_model, b2_model, ref_b2_model, b3_model, ref_b3_model):
        ...
        Returns:
          ...
          compression_optimizer: the optimizer for b3_model
          fusion_lr_scheduler: the learning rate decay scheduler for b3_model
        """
        if iteration > start_iter: 
            # The i-th phase (i>=2)
            ...
            if ref_b3_model is not None:
                ref_b3_model = ref_b3_model.to(self.device)
            ...
            ignored_params = list(map(id, b1_model.fc.parameters()))
            ignored_params2 = list(map(id, b1_model.fe_fc.parameters()))
            base_params = filter(lambda p: id(p) not in ignored_params, b1_model.parameters())
            base_params = filter(lambda p: id(p) not in ignored_params2, base_params)
            ...
            tg_params_new =[{'params': base_params, 'lr': branch1_lr, 'weight_decay': branch1_weight_decay}, \
                ...
                {'params': b1_model.fc.fc2.parameters(), 'lr': self.args.base_lr2, 'weight_decay': self.args.custom_weight_decay}, \
                #{'params': b1_model.fc.parameters(), 'lr': self.args.base_lr2, 'weight_decay': self.args.custom_weight_decay}, \
                {'params': b1_model.fe_fc.parameters(), 'lr': self.args.base_lr2, 'weight_decay': self.args.custom_weight_decay}]###
            ...
            # Set the optimizer for the aggregation weights, and b3_model
            b3_params = b3_model.parameters()
            b3_model = b3_model.to(self.device)
            if self.args.branch_mode == 'dual':
                ...
                compression_optimizer = optim.SGD(filter(lambda p: p.requires_grad, b3_params), lr=0.1, momentum=0.9)
            elif self.args.branch_mode == 'single':
                ...
                compression_optimizer = optim.SGD(filter(lambda p: p.requires_grad, b3_params), lr=0.0, momentum=0.9)
            else:
        ...
        if self.args.dataset == 'cifar100':
            if iteration > start_iter:
                tg_lr_scheduler = lr_scheduler.CosineAnnealingLR(optimizer=tg_optimizer, T_max=self.args.epochs)
            else:
                tg_lr_scheduler = lr_scheduler.CosineAnnealingLR(optimizer=tg_optimizer, T_max=self.args.init_epochs)
            fusion_lr_scheduler = lr_scheduler.MultiStepLR(fusion_optimizer, milestones=self.lr_strat, gamma=self.args.lr_factor)
            compression_lr_scheduler = lr_scheduler.CosineAnnealingLR(compression_optimizer, T_max=self.args.compression_epochs)
        elif self.args.dataset == 'imagenet_sub' or self.args.dataset == 'imagenet':
            if iteration > start_iter:
                tg_lr_scheduler = lr_scheduler.CosineAnnealingLR(optimizer=tg_optimizer, T_max=self.args.epochs)
            else:
        ...
        return tg_optimizer, tg_lr_scheduler, fusion_optimizer, fusion_lr_scheduler, compression_optimizer, compression_lr_scheduler
    ...
    def compute_acc(self, class_means, order, order_list, b1_model, b2_model, X_protoset_cumuls, Y_protoset_cumuls, \
        ...
        tg_feature_model = nn.Sequential(*list(b1_model.children())[:-2])###将-1改为-2
        ...
        ori_acc = compute_accuracy(self.args, self.save_path, self.fusion_vars, b1_model, b2_model, tg_feature_model, \
            current_means, X_protoset_cumuls, Y_protoset_cumuls, evalloader, \
            order_list, iteration, is_start_iteration=is_start_iteration)
        ...
        cumul_acc, each_class_acc = compute_accuracy(self.args, self.save_path, self.fusion_vars, b1_model, b2_model, tg_feature_model, \
            current_means, X_protoset_cumuls, Y_protoset_cumuls, evalloader, order_list, \
            iteration, is_start_iteration=is_start_iteration, compute_class_acc=True)
        # Add the results to the array, which stores all previous results
        ...
        return top1_acc_list_ori, top1_acc_list_cumul, each_class_acc
    ...
    def set_exemplar_set(self, b1_model, b2_model, is_start_iteration, iteration, last_iter, order, alpha_dr_herding, prototypes):
        ...
        Returns:
          ...
          class_means: the mean values for each class只在计算其他两种精度的时候用到
        ...
        num_features = b1_model.fc.in_features
        if self.args.dataset == 'cifar100':
            for iter_dico in range(last_iter*self.args.nb_cl, (iteration+1)*self.args.nb_cl):#range(0,50)
                # Set a temporary dataloader for the current class
                self.evalset.data = prototypes[iter_dico].astype('uint8')
                self.evalset.targets = np.zeros(self.evalset.data.shape[0])#(500,)
                evalloader = torch.utils.data.DataLoader(self.evalset, batch_size=self.args.eval_batch_size,
                    shuffle=False, num_workers=self.args.num_workers)
                num_samples = self.evalset.data.shape[0]#500
