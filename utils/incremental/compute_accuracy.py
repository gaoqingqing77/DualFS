...
def compute_accuracy(the_args, save_path, fusion_vars, b1_model, b2_model, tg_feature_model, class_means, \
    X_protoset_cumuls, Y_protoset_cumuls, evalloader, order_list, iteration, is_start_iteration=False, \
    compute_class_acc=False, scale=None, print_info=True, device=None, cifar=True, imagenet=False, \
    valdir=None):
    if device is None:
    ...
    correct_maml = 0
    total = 0
    class_accuracies = torch.zeros((iteration+1)*the_args.nb_cl, device=device)
    with torch.no_grad():
        for batch_idx, (inputs, targets) in enumerate(evalloader):
            ...
            if is_start_iteration:
                ...
                outputs, outputs_feature, _, _ = process_inputs_fp(the_args, fusion_vars, b1_model, b2_model, inputs)
            ...
            for idx in range(len(targets)):
                class_accuracies[targets[idx]] += (predicted[idx] == targets[idx])
    ...
    if compute_class_acc:
        class_sample_count = total/((iteration+1)*the_args.nb_cl)
        for i in range((iteration+1)*the_args.nb_cl):
            class_accuracies[i] /= class_sample_count

        performances = [f'Class {i} accuracy: {class_accuracies[i] * 100:.2f}%\n'
                        for i in range((iteration + 1) * the_args.nb_cl)]
        save_name = os.path.join(save_path, 'iter_{}_class_performances.txt'.format(iteration))
        with open(save_name, 'w') as file:
            for performance in performances:
                file.write(performance)
        print('Class performance has been saved to ', save_name, '/iter_{}_class_performances.txt'.format(iteration))
        each_class_acc = 100.*class_accuracies
        return [cnn_acc, icarl_acc, ncm_acc], each_class_acc
